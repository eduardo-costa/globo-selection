...
class HTTPServiceManager
{
	...
	/**
	 * Web services marked as persistent resides here.
	 */
	private var m_active : Map<String,BaseService>;
	...
	public function new() 
	{
		...
		m_active    = new Map<String,BaseService>();
	...
	private function RequestHandler(p_request : IncomingMessage, p_response : ServerResponse):Void
	{
		...
		var service_active : Bool   = m_active.exists(service_id);
		var service_alloc  : Bool   = true;
		
		Log("HTTP> RequestHandler url[" + request.url + "]["+method+"] id["+service_id+"] exists["+service_exists+"] active["+service_active+"] ip["+request.socket.remoteAddress+":"+request.socket.remotePort+"]", 1);						
		
		if (service_exists)
		{			
			if (service_active)
			{
				service = m_active.get(service_id);
				service_alloc = !service.persistent;
			}
						
			if (service_alloc)
			{
				trace("creating " + service_id);
				var c : Class<BaseService> = m_services.get(service_id);
				service = Type.createInstance(c, [this]);			
				m_active.set(service_id, service);
			}
